...
import { evaluateBenchmarkBlackBox } from './codex-v130-independent-evaluator.mjs';
...
function createFixtureReceiptSet(pack) {
  ...
  for (const task of manifest.tasks) {
    for (const lane of ['v129_deterministic_runtime', 'v130_direct_verified', 'v130_deterministic_orchestrated']) {
      writeFixtureAppServerReceipt(path.join(root, lane, `${task.taskId}.jsonl`), { inputTokens: 1000, outputTokens: 80 });
    }
  }
  return root;
}
...
function contractTests() {
  ...
  const bridgeV085Shape = bridgePrBodyProjection.v085PrProfileMetadataShape || {};
  ...
  const bridgeV085Body = [
    bridgeV085Shape.heading,
    ...(bridgeV085Shape.commands || []).map((command) => `- ${command}`),
  ].join('\n');
  const qualityGateWorkflow = fs.readFileSync('.github/workflows/quality-gate.yml', 'utf8');
  const weeklyHealthWorkflow = fs.readFileSync('.github/workflows/weekly-health-check.yml', 'utf8');
  const expectedCheckoutPin = 'actions/checkout@34e114876b0b11c390a56381ad16ebd13914f8d5';
  const expectedSetupNodePin = 'actions/setup-node@49933ea5288caeca8642d1e84afbd3f7d6820020';
  ...
  return [
    ...
    test('v130_performance_track_non_authoritative', () => source.performanceTrack?.state === 'deferred' && source.performanceTrack?.authority === 'non_authoritative' && source.performanceTrack?.sixtyTaskBenchmarkState === 'not_required_for_core_activation' && source.performanceTrack?.superiorityClaimState === 'not_proven' && source.performanceTrack?.skillRuntimeState === 'deferred' && source.performanceTrack?.dagAgentTeamState === 'deferred' && source.performanceTrack?.affectsQualityScore === false && source.performanceTrack?.affectsBlockingCount === false && source.performanceTrack?.affectsActivation === false && source.performanceTrack?.affectsRequiredChecks === false),
    test('v130_iris_funky_resume_policy_pass', () => source.targetDevelopmentResumePolicy?.state === 'active' && source.targetDevelopmentResumePolicy?.targetHarnessForProductDevelopment === '1.2.9' && source.targetDevelopmentResumePolicy?.v130TargetInstall === 'forbidden' && source.targetDevelopmentResumePolicy?.v130PerformanceTrackBlocksProductDevelopment === false && source.targetDevelopmentResumePolicy?.allowedUnderV129?.includes('bug_repair') && source.targetDevelopmentResumePolicy?.allowedUnderV129?.includes('small_code_change') && source.targetDevelopmentResumePolicy?.forbidden?.includes('fable_superiority_claim') && source.targetDevelopmentResumePolicy?.forbidden?.includes('authority_weakening') && source.targetDevelopmentResumePolicy?.forbidden?.includes('required_check_bypass')),
    test('v130_goal_contract_lite_present', () => {
      const required = ['goalStatement', 'acceptanceCriteria', 'nonGoals', 'allowedScope', 'forbiddenScope', 'requiredEvidence', 'stopCondition', 'repairBudget'];
      return source.goalContractLite?.state === 'active'
        && required.every((field) => source.goalContractLite.requiredFields?.includes(field))
        && source.goalContractLite.immutableAfterExecutionBegins === true
        && source.goalContractLite.scopeChangeInvalidatesReceiptAuthority === true
        && source.goalContractLite.artifactFamilyCreated === false;
    }),
    test('v130_loop_controller_lite_present', () => {
      const terminals = ['auto_merge', 'auto_repair', 'auto_rebase', 'auto_reject', 'auto_quarantine'];
      return source.loopControllerLite?.state === 'active'
        && source.loopControllerLite.candidateRepairMax === 1
        && source.loopControllerLite.sameBlockerMax === 1
        && source.loopControllerLite.providerTransientRetryMax === 1
        && source.loopControllerLite.noProgressWindow === 1
        && exact(source.loopControllerLite.terminalDispositions, terminals)
        && source.loopControllerLite.ownerApprovalWait === false
        && source.loopControllerLite.sameBlockerProhibitions?.includes('change_threshold');
    }),
    ...
    test('v130_workflow_markers_current_or_whitelisted', () => qualityGateWorkflow.startsWith('# CODEX_QUALITY_HARNESS_FILE v1.3.0') && weeklyHealthWorkflow.startsWith('# CODEX_QUALITY_HARNESS_FILE v1.3.0')),
    test('v130_target_overlay_template_not_installed_state', () => [source, docsManifest, activePolicy].every((manifest) => manifest.sourceCoreTargetRolloutState === 'not_started' && manifest.installedTargetHarnessVersion === '1.2.9' && manifest.operatorTargetHarnessDisplay === 'HARNESS v1.3.0 Core' && manifest.compatibilityAdapterInternalHarnessVersion === '1.2.9' && manifest.targetManifestOverlay?.projectionKind === 'profile_install_template_only' && manifest.targetManifestOverlay?.appliesOnlyAfterOwnerScopedProfileInstall === true && manifest.targetManifestOverlay?.mutatesTargetRepositories === false)),
    test('v130_active_policy_target_rollout_profile_adapter_scoped', () => !Object.hasOwn(activePolicy.profiles || {}, 'target_rollout') && activePolicy.profiles?.target_compatibility_profile_install?.activeOperatorAuthority === false && activePolicy.profiles?.target_compatibility_profile_install?.authority === 'internal_compatibility_adapter_reference_only' && exact(activePolicy.profiles?.target_compatibility_profile_install?.requiredReads || [], ['AGENTS.md', 'docs/process/CODEX_HARNESS_MANIFEST.json', 'compiled_instruction_envelope', 'owner_rollout_instruction']) && activePolicy.profiles?.target_compatibility_profile_install?.compatibilityReferenceReads?.['docs/process/CODEX_V129_SPEC.md'] === 'failing_compatibility_adapter_reference'),
    test('v130_source_no_package_json_npm_not_applicable', () => !fs.existsSync('package.json') && !fs.existsSync('package-lock.json')),
    test('v130_v129_shadow_cross_call_documented', () => activePolicy.profiles?.v129_shadow?.requiredReads?.includes('docs/process/CODEX_V129_SPEC.md') && activePolicy.profiles?.target_compatibility_profile_install?.deferredReads?.includes('docs/process/CODEX_V129_SPEC.md')),
    test('v130_action_pin_policy_consistent', () => [source, docsManifest, activePolicy].every((manifest) => manifest.actionPinPolicy?.state === 'active' && manifest.actionPinPolicy?.coreWorkflowsRequirePinnedCheckoutAndSetupNode === true && manifest.actionPinPolicy?.floatingCheckoutOrSetupNodeAllowedInCoreWorkflows === false && manifest.actionPinPolicy?.permissionWeakeningAllowed === false && manifest.actionPinPolicy?.pinnedActions?.checkout === expectedCheckoutPin && manifest.actionPinPolicy?.pinnedActions?.setupNode === expectedSetupNodePin) && qualityGateWorkflow.includes(expectedCheckoutPin) && qualityGateWorkflow.includes(expectedSetupNodePin) && weeklyHealthWorkflow.includes(expectedCheckoutPin) && weeklyHealthWorkflow.includes(expectedSetupNodePin) && !qualityGateWorkflow.includes('actions/checkout@v4') && !qualityGateWorkflow.includes('actions/setup-node@v4') && !weeklyHealthWorkflow.includes('actions/checkout@v4') && !weeklyHealthWorkflow.includes('actions/setup-node@v4')),
    ...
    test('v130_target_v085_pr_profile_metadata_projection_shape', () => bridgePrBodyProjection.state === 'display_only' && bridgePrBodyProjection.authorityCreated === false && bridgePrBodyProjection.finalDecisionAuthority === 'v1.1.8_final_decision_kernel' && bridgeV085Shape.requiredByExistingGate === 'v085StabilityStatus' && bridgeRequiredProfiles.every((profile) => bridgeV085Shape.appliesTo?.includes(profile)) && bridgeV085Body.includes('## Validation commands') && bridgeV085Body.includes('node scripts/codex-v130-self-test.mjs') && bridgeV085Body.includes('CODEX_QUALITY_REPORT=json node scripts/codex-local-quality-gate.mjs') && bridgeV085Body.includes('git diff --check') && bridgeV085Shape.bodyAuthority === 'display_only' && bridgeV085Shape.authorityCreated === false && bridgeV085Shape.productMutationCount === 0 && bridgePerformance.state === 'deferred' && bridgePerformance.superiorityClaimState === 'not_proven'),
...
function tokenDifferentialTests() {
  ...
  return [
    ...
    test('v130_hidden_validator_missing_fails', () => hiddenMissingResult.status === 'fail' && hiddenMissingResult.reasonCodes.includes('v130_hidden_validator_executable_missing')),
    test('v130_initially_green_repair_task_fails', () => initiallyGreen.status === 'fail' && initiallyGreen.reasonCodes.includes('v130_accepted_change_contract_not_met')),
