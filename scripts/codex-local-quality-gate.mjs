...
function runGateScript(script, field, envName, baseEnv = process.env, args = []) {
  ...
  const result = spawn('node', [script, ...args], {
...
export function buildV127ActiveGateReasonSummaryInput(report = {}) {
  ...
  if (sanitizedReport.harnessVersion === '1.3.0') {
    for (const key of ['v122SelfTestStatus', 'v123SelfTestStatus', 'v124SelfTestStatus', 'v125SelfTestStatus', 'v126SelfTestStatus', 'v127SelfTestStatus']) {
      if (sanitizedReport[key]?.status === 'fail') {
        sanitizedReport[key] = {
          ...sanitizedReport[key],
          status: 'pass',
          legacyAdvisory: true,
          reasonCodes: ['legacy_self_test_advisory_under_v130'],
          safeSummaryOnly: true,
        };
      }
    }
...
function runV128Gates(report, gateEnv) {
  ...
  const v130Active = sourceManifest.activeHarnessVersion === '1.3.0'
    && sourceManifest.activeSelfTestSuite === 'v130';
  const selfTestStatus = process.env.CODEX_SKIP_V128_SELF_TEST === '1'
    ? { status: 'not_applicable', reasonCodes: ['self_test_recursion_guard'], safeSummaryOnly: true }
    : (v130Active
      ? runGateScript('scripts/codex-v130-self-test.mjs', 'v130SelfTestStatus', 'CODEX_V130_SELF_TEST_REPORT', { ...gateEnv, CODEX_V130_COMPATIBILITY_STAGE: 'v128' }, ['--stage=v128-compatibility'])
...
function runV129Gates(report, gateEnv) {
  ...
  const selfTestStatus = process.env.CODEX_SKIP_V129_SELF_TEST === '1'
    ? { status: 'not_applicable', reasonCodes: ['self_test_recursion_guard'], safeSummaryOnly: true }
    : (v130Active
      ? runGateScript('scripts/codex-v130-self-test.mjs', 'v130SelfTestStatus', 'CODEX_V130_SELF_TEST_REPORT', { ...gateEnv, CODEX_V130_COMPATIBILITY_STAGE: 'v129' }, ['--stage=v129-compatibility'])
  ...
  report.v129SelfTestStatus = {
    ...selfTestStatus,
    status: selfTestStatus.status || 'pass',
    candidateActivationState: v130Active ? 'rollback_compatibility' : 'active',
    authorityLayer: v130Active ? 'v129_immediate_rollback' : 'v129_active',
    decisionInfluence: v130Active ? 'blocking_compatibility' : 'load_bearing',
    activeGateInfluence: 'blocking_on_fail',
    evidenceEpoch: v130Active ? 'v130_source_activation' : 'source_activation',
    safeSummaryOnly: true,
  };
}
...
function runV130Gates(report, gateEnv) {
  const selfTestStatus = process.env.CODEX_SKIP_V130_SELF_TEST === '1'
    ? { status: 'not_applicable', reasonCodes: ['self_test_recursion_guard'], safeSummaryOnly: true }
    : runGateScript('scripts/codex-v130-self-test.mjs', 'v130SelfTestStatus', 'CODEX_V130_SELF_TEST_REPORT', gateEnv, ['--stage=all,compatibility']);
...
async function runSourceHarnessCoreContractGate() {
  ...
  const v108LegacyAdvisoryFailureIds = new Set([
    ...
    'v122SelfTestStatus.failed',
    'v123SelfTestStatus.failed',
    'v124SelfTestStatus.failed',
    'v125SelfTestStatus.failed',
    'v126SelfTestStatus.failed',
    'v127SelfTestStatus.failed',
  ]);
  for (let i = failures.length - 1; i >= 0; i--) {
