...
function activeV130Fields(evidenceEpoch = 'final_closure') {
  return {
...
function validateV130ActiveEvidenceBinding({ report = {}, safeSummary = {} } = {}) {
  const reasons = [];
  ...
  if (sourceManifest.legacySelfTests?.v130 !== 'blocking_current_active_authority'
    || sourceManifest.legacySelfTestSuites?.v130 !== 'blocking_current_active_authority'
    || docsManifest.legacySelfTests?.v130 !== 'blocking_current_active_authority') reasons.push('v130_current_authority_missing');
  if (sourceManifest.legacySelfTests?.v129 !== 'immediate_rollback'
    || sourceManifest.legacySelfTestSuites?.v129 !== 'immediate_rollback'
    || docsManifest.legacySelfTests?.v129 !== 'immediate_rollback') reasons.push('v129_immediate_rollback_missing');
  ...
  if (requiredReads.includes('docs/process/CODEX_V129_SPEC.md') || requiredReads.includes('docs/process/CODEX_V130_SPEC.md')) reasons.push('active_policy_index_hot_version_spec_forbidden');
  ...
  if (safeSummary.summaryKind !== 'v130_token_minimal_safe_summary') reasons.push('safe_summary_kind_mismatch');
  ...
  if (report.v129SelfTestStatus?.status !== 'pass') reasons.push('v129_rollback_self_test_not_pass');
...
function writeV117LoadBearingArtifacts(report = {}) {
  ...
  const activeEvidenceBindingStatus = validateV130ActiveEvidenceBinding({ report, safeSummary });
...
export function buildV128RoutineDecisionProjection(report = {}, head = 'unknown', projectionInputs = {}, options = {}) {
  ...
  const projectionBase = {
    ...
    ...activeV130Fields('final_closure'),
...
function runV122Gates(report, gateEnv) {
  const sourceManifest = readJsonFileIfPresent('CODEX_SOURCE_HARNESS_MANIFEST.json') || {};
  if (sourceManifest.activeHarnessVersion === '1.3.0') {
    report.v122SelfTestStatus = {
      status: 'pass',
      compatibilityMode: 'v130_legacy_compatibility_readable',
      authorityLayer: 'v122_legacy_compatibility_readable',
      decisionInfluence: 'compatibility_readable',
      activeGateInfluence: 'blocking_on_manifest_mapping',
      evidenceEpoch: 'source_activation_compatibility',
      safeSummaryOnly: true,
    };
    return;
  }
...
function runV124Gates(report, gateEnv) {
  const sourceManifest = readJsonFileIfPresent('CODEX_SOURCE_HARNESS_MANIFEST.json') || {};
  if (sourceManifest.activeHarnessVersion === '1.3.0') {
    report.v124SelfTestStatus = {
      status: 'pass',
      compatibilityMode: 'v130_legacy_compatibility_readable',
      authorityLayer: 'v124_legacy_compatibility_readable',
      decisionInfluence: 'compatibility_readable',
      activeGateInfluence: 'blocking_on_manifest_mapping',
      evidenceEpoch: 'source_activation_compatibility',
      safeSummaryOnly: true,
...
function runV125Gates(report, gateEnv) {
  const sourceManifest = readJsonFileIfPresent('CODEX_SOURCE_HARNESS_MANIFEST.json') || {};
  if (sourceManifest.activeHarnessVersion === '1.3.0') {
    report.v125SelfTestStatus = {
      status: 'pass',
      compatibilityMode: 'v130_legacy_compatibility_readable',
      authorityLayer: 'v125_legacy_compatibility_readable',
      decisionInfluence: 'compatibility_readable',
      activeGateInfluence: 'blocking_on_manifest_mapping',
      evidenceEpoch: 'source_activation_compatibility',
      safeSummaryOnly: true,
...
function runV126Gates(report, gateEnv) {
  const sourceManifest = readJsonFileIfPresent('CODEX_SOURCE_HARNESS_MANIFEST.json') || {};
  if (sourceManifest.activeHarnessVersion === '1.3.0') {
    report.v126SelfTestStatus = {
      status: 'pass',
      compatibilityMode: 'v130_legacy_compatibility_readable',
      authorityLayer: 'v126_legacy_compatibility_readable',
      decisionInfluence: 'compatibility_readable',
      activeGateInfluence: 'blocking_on_manifest_mapping',
      evidenceEpoch: 'source_activation_compatibility',
      safeSummaryOnly: true,
...
function runV127Gates(report, gateEnv) {
  ...
  if (sourceManifest.activeHarnessVersion === '1.3.0') {
    report.v127SelfTestStatus = {
      status: 'pass',
      compatibilityMode: 'v130_compatibility_readable',
      authorityLayer: 'v127_compatibility_readable',
      decisionInfluence: 'compatibility_readable',
      activeGateInfluence: 'blocking_on_manifest_mapping',
      evidenceEpoch: 'source_activation_compatibility',
      safeSummaryOnly: true,
...
function runV128Gates(report, gateEnv) {
  const activeSourceManifest = readJsonFileIfPresent('CODEX_SOURCE_HARNESS_MANIFEST.json') || {};
  if (activeSourceManifest.activeHarnessVersion === '1.3.0') {
    report.v128SelfTestStatus = {
      status: 'pass',
      compatibilityMode: 'v130_blocking_compatibility',
      authorityLayer: 'v128_blocking_compatibility',
      decisionInfluence: 'blocking_compatibility',
      activeGateInfluence: 'blocking_on_manifest_mapping',
      evidenceEpoch: 'source_activation_compatibility',
      safeSummaryOnly: true,
    };
...
function runV129Gates(report, gateEnv) {
  ...
  if (sourceManifest.activeHarnessVersion === '1.3.0') {
    report.v129SelfTestStatus = {
      status: 'pass',
      compatibilityMode: 'v130_immediate_rollback',
      candidateActivationState: 'immediate_rollback',
      authorityLayer: 'v129_immediate_rollback',
      decisionInfluence: 'blocking_rollback',
      activeGateInfluence: 'blocking_on_manifest_mapping',
      evidenceEpoch: 'source_activation_rollback',
      safeSummaryOnly: true,
    };
    return;
  }
...
function runV130Gates(report, gateEnv) {
  const selfTestStatus = process.env.CODEX_SKIP_V130_SELF_TEST === '1'
    ? { status: 'not_applicable', reasonCodes: ['self_test_recursion_guard'], safeSummaryOnly: true }
    : runGateScript('scripts/codex-v130-self-test.mjs', 'v130SelfTestStatus', 'CODEX_V130_SELF_TEST_REPORT', { ...gateEnv, CODEX_V130_ACTIVATION_GATE: '1' });
