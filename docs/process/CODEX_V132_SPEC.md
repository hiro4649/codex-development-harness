...
Remote state, required checks, same-head binding, artifact integrity, and Final Decision are derived only from durable verification paths. The production GitHub collector accepts only repository/run identity and performs the GitHub API observation itself; caller-fed PR, check, artifact, head, conclusion, or timestamp data is rejected. It observes PR metadata and classic branch protection/GitHub Rulesets once, creates one stable required-check snapshot without volatile observation time in its binding digest, and shares that snapshot across all required workflow runs. A serialized GitHub receipt must be re-observed before use and is bound to repository, PR number, `pull_request` event, base/head SHA, workflow ID/path/content digest, reusable workflow reference when contracted, run ID/attempt, normalized required checks/workflows, and accepted-main artifact/workflow contracts.

A serialized Final Decision receipt must pass Ed25519 verification against a key ID and public-key fingerprint loaded through the accepted-main GitHub API trust root. Before reading that root, the collector observes the Source repository metadata and protected default branch through GitHub API. The observed default branch must be `main`; its exact HEAD must equal the recorded accepted-main SHA; the root embedded SHA/repository/default branch must match that observation; and the root path is fixed in code. Candidate commits, detached commits, alternate paths, candidate-selected keys, and cloned trust-root objects are rejected. The trust contract includes rotation and revocation state; this candidate cannot bootstrap its own authority.

Workflow contracts bind ID, path, exact content digest, and an optional reusable-workflow reference. Artifact archives are content-digested and their contracted JSON entry, schema, fields, semantic digest, repository, exact head, and pass status are observed. Artifact contracts are assigned to the workflow that produces them, so multi-run collection does not require every workflow to emit every artifact. Fixture constructors are separate and forbidden outside explicit test mode. Boolean claims such as `remoteChecksPass`, `sameHead`, or `artifactUploaded` are rejected. A PR body is display text, not evidence. Missing observation remains `not_observed`; only an authoritative billing annotation becomes `unavailable_billing`. A step-less run without that signal becomes `unavailable_pre_runner`, not a guessed billing or code failure.
